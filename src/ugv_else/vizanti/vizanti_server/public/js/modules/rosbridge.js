...
class Rosbridge {
	...
	connect(){
		...
		this.launches_client = new ROSLIB.Service({
			ros : this.ros,
			name : '/rosapi/launches',
			serviceType : 'vizanti_msgs/srv/Launches',
		});
	...
	async get_all_launches() {
		return new Promise(async (resolve) => {
			this.launches_client.callService(new ROSLIB.ServiceRequest({}), function (result) {
				resolve(result);
			});
		});
	}
